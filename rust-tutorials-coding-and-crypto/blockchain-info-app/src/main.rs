...
fn process_blockchain_info(address: &str) {
    ...
    let blockchain_address: BlockchainAddress = bc_info::blockhain_address_request(address);
    println!(
        "\n\nAnalyzing txs for Bitcoin address: {} ",
    ...
    if command.trim().eq("y") {
        ...
        for tx_id in &blockchain_address.txids {
            ...
            let blockchain_transaction: BlockchainTransaction =
                blockchain_transaction_request(&tx_id);
